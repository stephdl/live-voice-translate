...
Captures system audio (YouTube, Jitsi, etc.) and provides instant transcription and translation with interactive keyboard controls.
...
- ⌨️ **Keyboard shortcuts** - Pause, save, change mode on-the-fly
...
1. **Audio capture**: Monitors PulseAudio/PipeWire stream (YouTube, Jitsi, etc.)
...
- Keyboard controller uses `select()` for instant response
...
Shortcuts require terminal running in TTY mode. If piping output or running in non-interactive environment, use `--no-keyboard`:
```bash
...
For bugs or feature requests, please [open an issue](https://github.com/stephdl/live-voice-translate/issues).
